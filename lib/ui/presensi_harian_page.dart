...
class _PresensiHarianScreenState extends State<PresensiHarianScreen> {
  ...
  void _updateTime() {
    ...
    if (mounted) {
      setState(() {
        ...
        _currentDate = 'WIB - $dayName, ${now.day} $monthName ${now.year}';
      });
    }
  }
  ...
  Future<void> _fetchDashboardAndToday() async {
    try {
      ...
      _errorMessage = 'Gagal memuat data presensi.';
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final parsedData = data.map((item) {
          ...
          } else if (rawStatus.contains('alpha') || rawStatus.contains('tidak hadir') || rawStatus.contains('tidak_absen')) {
            statusTxt = 'Alpha';
  ...
  Future<void> _handleAbsenProcess(String tipe) async {
    // Basic validation
    if (tipe == 'masuk' && _isSudahAbsenMasuk) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Anda sudah melakukan absen masuk hari ini.')));
      return;
    }
    if (tipe == 'pulang' && !_isSudahAbsenMasuk) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Anda belum absen masuk.')));
      return;
    }
    if (tipe == 'pulang' && _isSudahAbsenKeluar) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Anda sudah melakukan absen keluar hari ini.')));
      return;
    }
    ...
    showDialog(
      ...
      builder: (context) => const Center(child: CircularProgressIndicator(color: Color(0xFF009688))),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: true,
        automaticallyImplyLeading: false, // We will build our own back button if needed, or leave it empty if inside bottom nav
  ...
  Widget _buildDigitalClock() {
    return Column(
      children: [
        Text(
          _currentTime,
          style: const TextStyle(fontSize: 56, fontWeight: FontWeight.bold, color: Color(0xFF0F172A), letterSpacing: 2),
        ),
  ...
  Widget _buildActionButtons() {
    bool canAbsenMasuk = !_isSudahAbsenMasuk && _isInRadius;
    bool canAbsenKeluar = _isSudahAbsenMasuk && !_isSudahAbsenKeluar && _isInRadius;

    return Row(
      children: [
        Expanded(
          child: GestureDetector(
            onTap: canAbsenMasuk ? () => _handleAbsenProcess('masuk') : null,
            child: Container(
              padding: const EdgeInsets.symmetric(vertical: 16),
              decoration: BoxDecoration(
                color: canAbsenMasuk ? const Color(0xFF009688) : Colors.grey.shade400,
              ...
              child: Row(
                ...
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.login_rounded, color: Colors.white, size: 16),
                  ),
                  const SizedBox(width: 8),
                  const Text('Absen Masuk', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
                ],
              ),
            ),
          ),
        ...
        Expanded(
          child: GestureDetector(
            onTap: canAbsenKeluar ? () => _handleAbsenProcess('pulang') : null,
            child: Container(
              padding: const EdgeInsets.symmetric(vertical: 16),
              decoration: BoxDecoration(
                color: canAbsenKeluar ? const Color(0xFFFEF2F2) : Colors.grey.shade100,
                border: Border.all(color: canAbsenKeluar ? const Color(0xFFFECACA) : Colors.grey.shade300, width: 2),
                borderRadius: BorderRadius.circular(16),
              ),
              child: Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Container(
                    padding: const EdgeInsets.all(6),
                    decoration: BoxDecoration(
                      color: canAbsenKeluar ? Colors.white : Colors.grey.shade200,
                      borderRadius: BorderRadius.circular(8),
                    ),
                    child: Icon(Icons.logout_rounded, color: canAbsenKeluar ? const Color(0xFFDC2626) : Colors.grey, size: 16),
                  ),
                  const SizedBox(width: 8),
                  Text(
                    'Absen Keluar', 
                    style: TextStyle(
                      color: canAbsenKeluar ? const Color(0xFFDC2626) : Colors.grey, 
                      fontWeight: FontWeight.bold, 
                      fontSize: 13
                    )
                  ),
  ...
  Widget _buildRingkasanKehadiran() {
    final Map<String, dynamic> items = {
      'Hadir': {'val': _attendanceSummary['hadir'] ?? 0, 'color': const Color(0xFF009688)},
      'Terlambat': {'val': _attendanceSummary['terlambat'] ?? 0, 'color': const Color(0xFFF59E0B)},
      'Izin': {'val': _attendanceSummary['izin'] ?? 0, 'color': const Color(0xFF3B82F6)},
      'Cuti': {'val': _attendanceSummary['cuti'] ?? 0, 'color': const Color(0xFF009688)},
      'Sakit': {'val': _attendanceSummary['sakit'] ?? 0, 'color': const Color(0xFF8B5CF6)},
      'Alpha': {'val': _attendanceSummary['alpha'] ?? 0, 'color': const Color(0xFFEF4444)},
    };
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Text(entry.key, style: const TextStyle(fontSize: 9, color: Color(0xFF64748B), fontWeight: FontWeight.w500)),
                      ],
                    ),
                    const SizedBox(height: 6),
                    Text('${entry.value['val']} Hari', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFF0F172A))),
                  ],
                ),
              );
            }).toList(),
          ...
          children: [
            ...
            InkWell(
              onTap: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => const RiwayatPresensiScreen()),
                );
              },
              child: const Text(
                'Lihat Semua',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11, color: Color(0xFF009688)),
              ),
            ),
          ],
        ),
